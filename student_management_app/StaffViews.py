...
from student_management_app.models import Attendance, AttendanceReport, CustomUser, FeedBackStaffs, SessionYearModel, Staffs, StudentResult, Students, Subjects, leaveReportStaff

def staff_home(request):
    user=CustomUser.objects.get(id=request.user.id)
    staff=Staffs.objects.get(admin=user)
    return render(request,"staff_template/staff_home_template.html",{"user":user,"staff":staff})
...
def staff_apply_leave_save(request):
    if request.method!="POST":
        ...
        staff_obj=Staffs.objects.get(admin=request.user.id)
        try:
            leave_report=leaveReportStaff(staff_id=staff_obj,leave_date=leave_date,leave_massage=leave_msg,leave_status=0) 
            leave_report.save() 
            messages.success(request,"Successfully Applied For Leave")
            return HttpResponseRedirect(reverse("staff_apply_leave"))
        except:
            messages.error(request,"Fail To Apply For Leave")
            return HttpResponseRedirect(reverse("staff_apply_leave"))
